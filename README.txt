...
The eventual goal for this project is a client-server platform for some old collectible card games (hence CCG) that my college friends and I used to love to play.  Given that this is done in my spare time with a son on the way, it will take a good number of years to finish.  I began work on this project in July 2011, and moved it from private to public repository in October 2011.


Project Goals:

Build a high-performance, heavily concurrent MMO-style platform for server-side services that is
	(1) 32 and 64 bit correct
	(2) EC2 hosted

Build a client using that cross-targets Win 8 (metro) and various tablets (not enough real estate on phones).


Learning Goals:

Full persistence implementation using batched ODBC.
...
The game targets are Legend of the Five Rings, Netrunner and Jyhad/Vampire : The Eternal Struggle.


Important Notes:

In order to be able to build and run the project, you'll need to follow the steps in README_Install.txt followed by README_Build.txt