...
#include <IPCore/Memory/Stl/Map.h>
...
IPSHARED_API class CConcurrencyManager
{
	public:
	...
	private:
		...
		void Enumerate_Processes( IP::Vector< std::shared_ptr< IManagedProcess > > &processes ) const;
		...
		void Handle_Message( EProcessID source_process_id, IP::UniquePtr< const Messaging::IProcessMessage > &message );
		void Handle_Get_Mailbox_By_ID_Request( EProcessID source_process_id, IP::UniquePtr< const Messaging::CGetMailboxByIDRequest > &message );
		void Handle_Get_Mailbox_By_Properties_Request( EProcessID source_process_id, IP::UniquePtr< const Messaging::CGetMailboxByPropertiesRequest > &message );
		void Handle_Add_New_Process_Message( EProcessID source_process_id, IP::UniquePtr< const Messaging::CAddNewProcessMessage > &message );
		void Handle_Shutdown_Process_Message( EProcessID source_process_id, IP::UniquePtr< const Messaging::CShutdownProcessMessage > &message );
		void Handle_Reschedule_Process_Message( EProcessID source_process_id, IP::UniquePtr< const Messaging::CRescheduleProcessMessage > &message );
		void Handle_Release_Mailbox_Response( EProcessID source_process_id, IP::UniquePtr< const Messaging::CReleaseMailboxResponse > &message );
		void Handle_Shutdown_Self_Response( EProcessID source_process_id, IP::UniquePtr< const Messaging::CShutdownSelfResponse > &message );
		void Handle_Shutdown_Manager_Message( EProcessID source_process_id, IP::UniquePtr< const Messaging::CShutdownManagerMessage > &message );
		...
		void Send_Process_Message( EProcessID dest_process_id, IP::UniquePtr< Messaging::IProcessMessage > &message );
		...
		// Types
		using GetMailboxByPropertiesRequestCollectionType = IP::MultiMap< EProcessID, IP::UniquePtr< const Messaging::CGetMailboxByPropertiesRequest > >;
		...
		using ProcessRecordTableType = IP::UnorderedMap< EProcessID, std::shared_ptr< CProcessRecord > >;
		...
		IP::UniquePtr< IP::Time::CTimeKeeper > TimeKeeper;

		IP::UniquePtr< tbb::task_scheduler_init > TBBTaskSchedulerInit;
