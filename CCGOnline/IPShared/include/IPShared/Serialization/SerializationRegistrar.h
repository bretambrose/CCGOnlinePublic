...
IPSHARED_API class CSerializationRegistrar
{
	public:
		...
		static void Build_Binding_Set( CTypeSerializationDefinition *definition, const IP::String &name, U T::* member_pointer, bool allow_polymorphism );
	...
	private:
		...
		static void Register_Factory_Set_For_Type()
		{
			...
			CSerializerFactorySet *factory_set = IP::New< CSerializerFactorySet >( MEMORY_TAG, CGetOrBuildXMLSerializer< T >() );
		...
		static XML::IXMLSerializer *Build_Composite_Serializer( const Loki::TypeInfo &type_info )
		{
			...
			auto serializer = IP::New< XML::CCompositeXMLSerializer >( MEMORY_TAG );
		...
		template< typename T >
		class CGetOrBuildXMLSerializer< IP::Vector< T > >
		{
			public:

				XML::IXMLSerializer* operator()( bool allow_polymorphism ) const {
					Loki::TypeInfo type_info( typeid( IP::Vector< T > ) );
					auto iter = XMLSerializers.find( type_info );
					if( iter != XMLSerializers.end() )
					{
						return iter->second;
					...
					XML::IXMLSerializer *vector_serializer = IP::New< XML::CVectorXMLSerializer >( MEMORY_TAG, serializer, Prep_Vector_For_Read< T > );
				...
				XML::IXMLSerializer* operator()( bool allow_polymorphism ) const {
					...
					if ( allow_polymorphism && type_iter != TypeSerializationDefinitions.end() )
					{
						...
						if ( most_derived_type_definition != nullptr )
						{
							...
							auto pointer_serializer = IP::New< CPointerXMLSerializer >( MEMORY_TAG, derived_serializer, most_derived_type_definition->Get_Pointer_Prep_Delegate() );
					...
					XML::IXMLSerializer *serializer = CGetOrBuildXMLSerializer< T >()( allow_polymorphism );
					XML::IXMLSerializer *pointer_serializer = IP::New< CPointerXMLSerializer >( MEMORY_TAG, serializer, Prep_Pointer_For_Read< T > );
		...
		static void Build_Derived_Type_List( const Loki::TypeInfo &type_info, IP::Vector< Loki::TypeInfo > &derived_types );

		using TypeSerializationDefinitionTableType = IP::UnorderedMap< Loki::TypeInfo, CTypeSerializationDefinition *, STypeInfoContainerHelper >;
		using FactorySetTableType = IP::UnorderedMap< Loki::TypeInfo, CSerializerFactorySet *, STypeInfoContainerHelper >;
		using XMLSerializerTableType = IP::UnorderedMap< Loki::TypeInfo, XML::IXMLSerializer *, STypeInfoContainerHelper >;
		using DerivedClassTableType = IP::UnorderedMap< Loki::TypeInfo, IP::Vector< Loki::TypeInfo >, STypeInfoContainerHelper >;

		using PolymorphicTypesTableType = IP::UnorderedMap< Loki::TypeInfo, Loki::TypeInfo, STypeInfoContainerHelper >;
		using PolymorphicEnumTableType = IP::UnorderedMap< uint64_t, Loki::TypeInfo >;
		using PolymorphicEnumTablesTableType = IP::UnorderedMap< Loki::TypeInfo, PolymorphicEnumTableType *, STypeInfoContainerHelper >;
...
#define REGISTER_ENUM_SERIALIZER( e ) IP::Serialization::CSerializationRegistrar::Register_Primitive_XML_Serializer( Loki::TypeInfo( typeid( e ) ), IP::New< IP::Serialization::XML::CEnumXMLSerializer< e > >( MEMORY_TAG ) );
...
template< typename T, typename U >
void CSerializationRegistrar::Build_Binding_Set( CTypeSerializationDefinition *definition, const IP::String &name, U T::* member_pointer, bool allow_polymorphism )
{
...
void CSerializationRegistrar::Register_Polymorphic_Enum_Entry( T enum_entry, const Loki::TypeInfo &type_info )
{
	...
	{
		auto enum_table = IP::New< PolymorphicEnumTableType >( MEMORY_TAG );
