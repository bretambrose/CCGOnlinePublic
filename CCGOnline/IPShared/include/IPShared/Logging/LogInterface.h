...
IPSHARED_API class CLogInterface
{
	public:

		// Initialization and shut down
		// There is a two-level init process to better support unit testing.  It could probably be consolidated though.
		static void Initialize_Static( const IP::String &service_name, ELogLevel log_level );
		...
		static const IP::String &Get_Service_Name( void ) { return ServiceName; }
		static const IP::String &Get_Log_Path( void ) { return LogPath; }
		static const IP::String &Get_Archive_Path( void ) { return ArchivePath; }

		// Functions to actually log information to a file
		static void Log( IP::String &message );
		static void Log( IP::String &&message );
		static void Log( const IP::String &message );
		static void Log( const IP::OStringStream &message_stream );
		static void Log( const char *message );

		static void LogAssert( IP::String &message ) { Log( message ); }

		static std::shared_ptr< IP::Execution::IManagedProcess > Get_Logging_Process( void ) { return LogProcess; }

	private:
		
		static IP::String ServiceName;

		static std::mutex LogLock;

		static std::shared_ptr< IP::Execution::IManagedProcess > LogProcess;

		static IP::String LogPath;
		static IP::String LogSubdirectory;
		static IP::String ArchivePath;
		static IP::String ArchiveSubdirectory;
...
#define LOG( log_level, stream_expression ) if ( IP::Logging::CLogInterface::Get_Log_Level() >= log_level ) { IP::OStringStream log_stream; log_stream << stream_expression; IP::Logging::CLogInterface::Log( log_stream ); }
