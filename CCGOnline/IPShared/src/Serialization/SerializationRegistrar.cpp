...
#include <IPCore/Memory/Stl/Stack.h>
...
void CSerializationRegistrar::Add_Inheritance_Relationship( const Loki::TypeInfo &derived_type_info, const Loki::TypeInfo &base_type_info )
{
	...
	{
		DerivedClassTableType::value_type entry( base_type_info, IP::Vector< Loki::TypeInfo >() );
...
XML::IXMLSerializer *CSerializationRegistrar::Get_Or_Build_Polymorphic_Enum_Serializer( const Loki::TypeInfo &type_info )
{
	...
	auto poly_serializer = IP::New< XML::CEnumPolymorphicXMLSerializer >( MEMORY_TAG, enum_type_info );
	...
	for ( const auto &table_iter : *table )
	{
		...
		if ( pointer_iter == XMLSerializers.end() )
		{
			...
			auto pointer_serializer = IP::New< XML::CPointerXMLSerializer >( MEMORY_TAG, entry_iter->second, type_definition->Get_Pointer_Prep_Delegate() );
...
void CSerializationRegistrar::Build_Derived_Type_List( const Loki::TypeInfo &type_info, IP::Vector< Loki::TypeInfo > &derived_types )
{
	IP::Stack< Loki::TypeInfo > type_stack;
...
void CSerializationRegistrar::Finalize( void )
{
	...
	for ( const auto &type_entry : TypeSerializationDefinitions )
	{
		...
		auto pointer_serializer = IP::New< XML::CPointerXMLSerializer >( MEMORY_TAG, serializer, type_definition->Get_Pointer_Prep_Delegate() );
