...
#include <IPDatabase/DatabaseProcessBase.h>

#include <IPCore/Debug/DebugAssert.h>
#include <IPDatabase/DatabaseProcessMessages.h>
...
CDatabaseProcessBase::CDatabaseProcessBase( IDatabaseEnvironment *environment, const IP::String &connection_string, bool process_task_results_locally, const SProcessProperties &properties ) :
...
void CDatabaseProcessBase::Per_Frame_Logic_End( void )
{
	...
	for ( auto success_iter = successes.cbegin(), end = successes.cend(); success_iter != end; ++success_iter )
	{
		...
		{
			auto response = IP::Make_Process_Message< Messaging::CRunDatabaseTaskResponse >( MEMORY_TAG, pending_request_iter->second.second, true );
	...
	for ( auto failure_iter = failures.cbegin(), end = failures.cend(); failure_iter != end; ++failure_iter )
	{
		...
		{
			auto response = IP::Make_Process_Message< Messaging::CRunDatabaseTaskResponse >( MEMORY_TAG, pending_request_iter->second.second, false );
			Send_Process_Message( pending_request_iter->second.first, response );
		}
...
void CDatabaseProcessBase::Handle_Run_Database_Task_Request( EProcessID process_id, IP::UniquePtr< const Messaging::CRunDatabaseTaskRequest > &message )
{
	IDatabaseTask *task = message->Get_Task().get();
