...
CCompoundDatabaseTaskBase::~CCompoundDatabaseTaskBase()
{
	...
	std::for_each( Children.begin(), Children.end(), []( const DBTaskListTablePairType& pair ){ IP::Delete( pair.second ); } );
	Children.clear();
}
...
void CCompoundDatabaseTaskBase::Add_Child_Task( IDatabaseTask *task )
{
	...
	{
		task_list = IP::New< DBTaskListType >( MEMORY_TAG );
...
void CCompoundDatabaseTaskBase::Clear_Child_Tasks( void )
{
	for( auto iter = Children.begin(); iter != Children.end(); ++iter)
	{
		...
		std::for_each( task_list->begin(), task_list->end(), []( IDatabaseTask *task ){ IP::Delete( task ); } );
		task_list->clear();
	}
}
