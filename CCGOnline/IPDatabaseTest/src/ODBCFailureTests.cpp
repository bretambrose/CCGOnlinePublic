...
#include <IPDatabase/Interfaces/CompoundDatabaseTaskBatchInterface.h>
...
#include <IPDatabaseTest/ODBCShared.h>
...
class CMissingProcedureCall : public TDatabaseProcedureCall< CEmptyVariableSet, ISIZE, CEmptyVariableSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.missing_procedure"; }

		void Verify_Results( uint32_t expected_rollback_count ) 
		{
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_MissingProcedureCall_Test( uint32_t task_count )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CMissingProcedureCall< ISIZE, OSIZE > > db_task_batch;
	IP::Vector< CMissingProcedureCall< ISIZE, OSIZE > * > tasks;
	...
	{
		uint32_t expected_rollback_count = i % ISIZE;	// this is probably unreliable across ODBC impl; it assumes the task list gets repeatedly rolled back in sequential order
		tasks[ i ]->Verify_Results( expected_rollback_count );
		IP::Delete( tasks[ i ] );
	}

	CODBCFactory::Get_Environment()->Shutdown_Connection( connection->Get_ID() );
	IP::Delete( connection );
}
...
class CTooFewInputParams : public CODBCVariableSet
{
	public:
		...
		virtual void Get_Variables( IP::Vector< IDatabaseVariable * > &variables )
		{
...
template< typename IPARAMS, uint32_t ISIZE, uint32_t OSIZE >
class CWrongArityProcedureCall : public TDatabaseProcedureCall< IPARAMS, ISIZE, CEmptyVariableSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.arity_failure"; }
...
template< typename IPARAMS, uint32_t ISIZE, uint32_t OSIZE >
void Run_WrongArityProcedureCall_Test( uint32_t task_count )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CWrongArityProcedureCall< IPARAMS, ISIZE, OSIZE > > db_task_batch;
	IP::Vector< CWrongArityProcedureCall< IPARAMS, ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
...
class CBadParamConversionProcedureCall : public TDatabaseProcedureCall< CInconvertibleProcedureParams, ISIZE, CEmptyVariableSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.bad_input_params"; }
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_BadParamConversionProcedureCall_Test( uint32_t task_count )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CBadParamConversionProcedureCall< ISIZE, OSIZE > > db_task_batch;
	IP::Vector< CBadParamConversionProcedureCall< ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
...
class CBadParamConversionFunctionCall : public TDatabaseFunctionCall< CInconvertibleFunctionParams, ISIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.bad_function_return"; }
...
template< uint32_t ISIZE >
void Run_BadParamConversionFunctionCall_Test( uint32_t task_count )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CBadParamConversionFunctionCall< ISIZE > > db_task_batch;
	IP::Vector< CBadParamConversionFunctionCall< ISIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
...
class CBadResultSetConversionProcedureCall : public TDatabaseProcedureCall< CEmptyVariableSet, ISIZE, CInconvertibleResultSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.bad_result_set_conversion"; }
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_BadResultSetConversionProcedureCall_Test( uint32_t task_count )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CBadResultSetConversionProcedureCall< ISIZE, OSIZE > > db_task_batch;
	IP::Vector< CBadResultSetConversionProcedureCall< ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
...
class CFunctionInputProcedureCall : public TDatabaseFunctionCall< CFunctionInputProcedureSet, ISIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.function_input_procedure"; }
...
template< uint32_t ISIZE >
void Run_FunctionInputProcedureCall_Test( uint32_t task_count )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CFunctionInputProcedureCall< ISIZE > > db_task_batch;
	IP::Vector< CFunctionInputProcedureCall< ISIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
...
class CProcedureInputFunctionCall : public TDatabaseProcedureCall< CProcedureInputFunctionSet, ISIZE, CEmptyVariableSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.procedure_input_function"; }
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_ProcedureInputFunctionCall_Test( uint32_t task_count )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CProcedureInputFunctionCall< ISIZE, OSIZE > > db_task_batch;
	IP::Vector< CProcedureInputFunctionCall< ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
...
class CThrowExceptionProcedureCall : public TDatabaseProcedureCall< CExceptionInputParams, ISIZE, CExceptionOutputParams, OSIZE >
{
	public:
		...
		CThrowExceptionProcedureCall( const char *proc_name, bool throw_exception ) : 
			BASECLASS(),
			ProcName( proc_name ),
		...
		virtual const char *Get_Database_Object_Name( void ) const { return ProcName; }

		void Verify_Results( uint32_t self_index, uint32_t exception_index1, uint32_t exception_index2 ) 
		{
			FATAL_ASSERT( exception_index1 < exception_index2 );

			uint32_t expected_rollbacks = 0;
			uint32_t my_batch = self_index / ISIZE;
	...
	protected:
		...
		virtual void On_Fetch_Results( IDatabaseVariableSet *result_set, int64_t rows_fetched ) 
		{
			...
			for ( int64_t i = 0; i < rows_fetched; ++i )
			{
	...
	private:
		...
		IP::Vector< CExceptionOutputParams > Results;
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_ThrowExceptionProcedureCall_Test( const char *proc_name, uint32_t task_count, uint32_t exception_index1, uint32_t exception_index2 )
{
	...
	IP::Vector< CThrowExceptionProcedureCall< ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
	...
	uint32_t exception_count = ( exception_index2 < task_count ) ? 2 : 1;
...
{
	Run_ThrowExceptionProcedureCall_Test< 1, 1 >( "dynamic.exception_thrower", 1, 0, 666 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_2_2_2_0_NULL )
{
	Run_ThrowExceptionProcedureCall_Test< 2, 2 >( "dynamic.exception_thrower", 2, 0, 666 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_2_2_2_1_NULL )
{
	Run_ThrowExceptionProcedureCall_Test< 2, 2 >( "dynamic.exception_thrower", 2, 1, 666 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_2_2_5_1_NULL )
{
	Run_ThrowExceptionProcedureCall_Test< 2, 2 >( "dynamic.exception_thrower", 5, 1, 666 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_3_0_NULL )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 3, 0, 666 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_3_1_NULL )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 3, 1, 666 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_3_2_NULL )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 3, 2, 666 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_7_2_NULL )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 7, 2, 666 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_2_2_2_0_1 )
{
	Run_ThrowExceptionProcedureCall_Test< 2, 2 >( "dynamic.exception_thrower", 2, 0, 1 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_3_0_1 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 3, 0, 1 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_3_0_2 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 3, 0, 2 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_3_1_2 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 3, 1, 2 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_6_0_1 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 6, 0, 1 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_6_1_2 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 6, 1, 2 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_6_0_2 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 6, 0, 2 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_6_0_3 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 6, 0, 3 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_6_1_4 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 6, 1, 4 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_6_2_5 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 6, 2, 5 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_6_3_4 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 6, 3, 4 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_6_3_5 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 6, 3, 5 );
}

TEST_F( ODBCFailureTests, UserExceptionProcedureCall_3_2_6_4_5 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.exception_thrower", 6, 4, 5 );
}
...
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.unique_constraint_violator", 6, 0, 1 );
}

TEST_F( ODBCFailureTests, UniquenessViolationProcedureCall_3_2_6_1_2 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.unique_constraint_violator", 6, 1, 2 );
}

TEST_F( ODBCFailureTests, UniquenessViolationProcedureCall_3_2_6_0_2 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.unique_constraint_violator", 6, 0, 2 );
}

TEST_F( ODBCFailureTests, UniquenessViolationProcedureCall_3_2_6_0_3 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.unique_constraint_violator", 6, 0, 3 );
}

TEST_F( ODBCFailureTests, UniquenessViolationProcedureCall_3_2_6_1_4 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.unique_constraint_violator", 6, 1, 4 );
}

TEST_F( ODBCFailureTests, UniquenessViolationProcedureCall_3_2_6_2_5 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.unique_constraint_violator", 6, 2, 5 );
}

TEST_F( ODBCFailureTests, UniquenessViolationProcedureCall_3_2_6_3_4 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.unique_constraint_violator", 6, 3, 4 );
}

TEST_F( ODBCFailureTests, UniquenessViolationProcedureCall_3_2_6_3_5 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.unique_constraint_violator", 6, 3, 5 );
}

TEST_F( ODBCFailureTests, UniquenessViolationProcedureCall_3_2_6_4_5 )
{
	Run_ThrowExceptionProcedureCall_Test< 3, 2 >( "dynamic.unique_constraint_violator", 6, 4, 5 );
}
...
class CMissingSelectProcedureCall : public TDatabaseProcedureCall< CMissingSelectParams, ISIZE, CMissingSelectResultSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.skip_select_procedure"; }
	...
	private:
		...
		IP::Vector< CMissingSelectResultSet > Results;
		uint64_t AccountCount;
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_MissingSelectProcedureCall_Test( uint32_t task_count, uint32_t skip_index1, uint32_t skip_index2 )
{
	...
	IP::Vector< CMissingSelectProcedureCall< ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
	...
	uint32_t exception_count = ( skip_index2 < task_count ) ? 2 : 1;
...
class CExtraSelectProcedureCall : public TDatabaseProcedureCall< CExtraSelectParams, ISIZE, CEmptyVariableSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.extra_select_procedure"; }
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_ExtraSelectProcedureCall_Test( uint32_t task_count, uint32_t non_extra_index )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CExtraSelectProcedureCall< ISIZE, OSIZE > > db_task_batch;
	IP::Vector< CExtraSelectProcedureCall< ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
	...
	uint32_t bad_task_count = ( non_extra_index < task_count ) ? task_count - 1 : task_count;
...
class CTooManyResultsProcedureCall : public TDatabaseProcedureCall< CTooManyResultsParams, ISIZE, CTooManyResultsResultSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.too_many_results_procedure"; }
	...
	private:
		...
		IP::Vector< CTooManyResultsResultSet > Results;
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_TooManyResultsProcedureCall_Test( uint32_t task_count, uint32_t extra_index )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CTooManyResultsProcedureCall< ISIZE, OSIZE > > db_task_batch;
	IP::Vector< CTooManyResultsProcedureCall< ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
	...
	uint32_t good_task_count = ( extra_index < task_count ) ? task_count - 1 : task_count;
...
class CExtraColumnProcedureCall : public TDatabaseProcedureCall< CExtraColumnParams, ISIZE, CExtraColumnResultSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.extra_column_procedure"; }
	...
	private:
		...
		IP::Vector< CExtraColumnResultSet > Results;
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_ExtraColumnProcedureCall_Test( uint32_t task_count, uint32_t extra_index )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CExtraColumnProcedureCall< ISIZE, OSIZE > > db_task_batch;
	IP::Vector< CExtraColumnProcedureCall< ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
...
class CMissingColumnProcedureCall : public TDatabaseProcedureCall< CMissingColumnParams, ISIZE, CMissingColumnResultSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.missing_column_procedure"; }
	...
	private:
		...
		IP::Vector< CMissingColumnResultSet > Results;
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_MissingColumnProcedureCall_Test( uint32_t task_count, uint32_t missing_index )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CMissingColumnProcedureCall< ISIZE, OSIZE > > db_task_batch;
	IP::Vector< CMissingColumnProcedureCall< ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
	...
	uint32_t good_task_count = ( missing_index < task_count ) ? task_count - 1 : task_count;
...
class CInvalidResultConversionProcedureCall : public TDatabaseProcedureCall< CInvalidResultConversionParams, ISIZE, CInvalidResultConversionResultSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.invalid_result_conversion_procedure"; }
	...
	private:
		...
		IP::Vector< CInvalidResultConversionResultSet > Results;
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_InvalidResultConversionProcedureCall_Test( uint32_t task_count, uint32_t invalid_index )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CInvalidResultConversionProcedureCall< ISIZE, OSIZE > > db_task_batch;
	IP::Vector< CInvalidResultConversionProcedureCall< ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
	...
	uint32_t good_task_count = ( invalid_index < task_count ) ? task_count - 1 : task_count;
...
class CResultStringTruncationProcedureCall : public TDatabaseProcedureCall< CResultStringTruncationParams, ISIZE, CResultStringTruncationResultSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.string_truncation_procedure"; }
	...
	private:
		...
		IP::Vector< CResultStringTruncationResultSet > Results;
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_ResultStringTruncationProcedureCall_Test( uint32_t task_count, uint32_t invalid_index )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CResultStringTruncationProcedureCall< ISIZE, OSIZE > > db_task_batch;
	IP::Vector< CResultStringTruncationProcedureCall< ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
...
class CSelectAccountBadTableTask : public TDatabaseSelect< CSelectAccountBadTableResultSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.accounts_bad"; }
		virtual void Build_Column_Name_List( IP::Vector< const char * > &column_names ) const
		{
			column_names.push_back( "account_id" );
			column_names.push_back( "account_email" );
			column_names.push_back( "nickname" );
			column_names.push_back( "nickname_sequence_id" );
		}
	...
	private:
		...
		IP::Vector< CSelectAccountBadTableResultSet > Results;
...
template< uint32_t OSIZE >
void Run_SelectAccountBadTable_Test( uint32_t task_count )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CSelectAccountBadTableTask< OSIZE > > db_task_batch;
	IP::Vector< CSelectAccountBadTableTask< OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
...
class CSelectAccountBadColumnTask : public TDatabaseSelect< CSelectAccountBadColumnResultSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.accounts"; }
		virtual void Build_Column_Name_List( IP::Vector< const char * > &column_names ) const
		{
			column_names.push_back( "account_ids" );
			column_names.push_back( "account_email" );
			column_names.push_back( "nickname" );
			column_names.push_back( "nickname_sequence_id" );
		}
	...
	private:
		...
		IP::Vector< CSelectAccountBadColumnResultSet > Results;
...
template< uint32_t OSIZE >
void Run_SelectAccountBadColumn_Test( uint32_t task_count )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CSelectAccountBadColumnTask< OSIZE > > db_task_batch;
	IP::Vector< CSelectAccountBadColumnTask< OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
...
class CSelectAccountBadConversionTask : public TDatabaseSelect< CSelectAccountBadConversionResultSet, OSIZE >
{
	...
	private:
		...
		IP::Vector< CSelectAccountBadConversionResultSet > Results;
...
template< uint32_t OSIZE >
void Run_SelectAccountBadConversion_Test( uint32_t task_count )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CSelectAccountBadConversionTask< OSIZE > > db_task_batch;
	IP::Vector< CSelectAccountBadConversionTask< OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
...
class CTVFBadColumnParams : public CODBCVariableSet
{
	public:
		...
		CTVFBadColumnParams( uint64_t account_id ) :
			BASECLASS(),
...
class CBadColumnTableValuedFunctionTask : public TDatabaseTableValuedFunctionCall< CTVFBadColumnParams, ISIZE, CTVFBadColumnResultSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.tabled_valued_function"; }
		virtual void Build_Column_Name_List( IP::Vector< const char * > &column_names ) const
		{
			column_names.push_back( "product_desc" );
			column_names.push_back( "product_key_descrip" );
		}
	...
	private:
		...
		IP::Vector< CTVFBadColumnResultSet > Results;
...
template< uint32_t ISIZE, uint32_t OSIZE >
void Run_BadColumnTableValuedFunctionTest( uint32_t task_count, uint64_t account_id )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );

	TDatabaseTaskBatch< CBadColumnTableValuedFunctionTask< ISIZE, OSIZE > > db_task_batch;
	IP::Vector< CBadColumnTableValuedFunctionTask< ISIZE, OSIZE > * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
...
class CFailableCompoundInsertParams : public CODBCVariableSet
{
	public:
		...
		CFailableCompoundInsertParams( uint64_t id, uint64_t user_data, bool should_fail ) :
			BASECLASS(),
			ID( id ),
			UserData( user_data ),
...
class CFailableCompoundInsertProcedureCall : public TDatabaseProcedureCall< CFailableCompoundInsertParams, ISIZE, CEmptyVariableSet, 1 >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.test_failable_compound_insert"; }
...
class CFailableCompoundInsertCountParams : public CODBCVariableSet
{
	public:
		...
		CFailableCompoundInsertCountParams( uint64_t id, bool should_fail ) :
			BASECLASS(),
			ID( id ),
			ShouldFail( should_fail )
		{}
...
class CFailableCompoundInsertCountProcedureCall : public TDatabaseProcedureCall< CFailableCompoundInsertCountParams, ISIZE, CFailableCompoundInsertCountResultSet, OSIZE >
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return "dynamic.test_failable_compound_insert_count"; }
...
class CFailableCompoundInsertTask : public TCompoundDatabaseTask< BATCH_SIZE >
{
	public:
		...
		{
			Register_Child_Type_Success_Callback( Loki::TypeInfo( typeid( Child1Type ) ), std::bind(&CFailableCompoundInsertTask::On_Insert_Success, this) );
		}
...
void Run_FirstFailableCompoundInsertTask_Test( uint32_t task_count, uint32_t bad_task, uint32_t bad_first_child_index )
{
	IDatabaseConnection *connection = CODBCFactory::Get_Environment()->Add_Connection( "Driver={SQL Server Native Client 11.0};Server=AZAZELPC\\CCGONLINE;Database=testdb;UID=testserver;PWD=TEST5erver#;", false );
	ASSERT_TRUE( connection != nullptr );
	...
	IP::Vector< CompoundTaskType * > tasks;
	for ( uint32_t i = 0; i < task_count; ++i )
	{
