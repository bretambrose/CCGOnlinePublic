...
class TCompoundDatabaseTaskBatch : public ICompoundDatabaseTaskBatch
{
	public:
		...
		virtual ~TCompoundDatabaseTaskBatch()
		{
			...
			std::for_each( ChildCallContexts.begin(), ChildCallContexts.end(), []( ChildCallContextPair &pair ){ IP::Delete( pair.second ); } );
	...
	private:
		...
		using ChildTypeVector = IP::Vector< Loki::TypeInfo >;
		using ChildCallContextTable = IP::UnorderedMap< Loki::TypeInfo, IDatabaseCallContext *, STypeInfoContainerHelper >;
...
{
   IDatabaseCallContext *child_call_context = IP::New< CDatabaseCallContext< T::InputParametersType, T::InputParameterBatchSize, T::ResultSetType, T::ResultSetBatchSize > >( MEMORY_TAG );
