...
static void Log_DB_Error( IDatabaseStatement *statement, IDatabaseVariableSet *input_params, const char *additional_error_context )
{
	...
	{
		LOG( ELogLevel::LL_LOW, additional_error_context );
	}
...
static void Extract_Batch_Error( IDatabaseCallContext *call_context, 
											...
											const char *additional_error_context, 
...
void Execute_Task_List( IDatabaseCallContext *call_context, IDatabaseStatement *statement, const DBTaskListType &sub_list, EExecuteDBTaskListResult &result, DBTaskListType::const_iterator &first_failed_task )
{
	...
	const char *additional_error_context = NULL;		// error logging is somewhat removed from error recognition, so use this awkward way of forwarding user-level information
	while ( fetch_status != FRST_ERROR && fetch_status != FRST_FINISHED_ALL )
	{
		if ( iter == sub_list.cend() )
		{
			...
			additional_error_context = "\tThere were more result sets than batched calls.";
			break;
		}
		...
		else if ( fetch_status == FRST_FINISHED_ALL )
		{
			...
			else
			{
				...
				*/
				fetch_status = FRST_ERROR;
				additional_error_context = "\tThere were fewer result sets than batched calls.";
			}
		}
	}
