...
SODBCError::SODBCError( int32_t sql_error_code, char *sql_state, const IP::String &error_description ) :
...
bool CODBCObjectBase::Refresh_Error_Status( SQLRETURN error_code )
{
	...
	SQLCHAR error_buffer[ 1024 ] = { 0 };

	for ( SQLSMALLINT i = 1; i <= record_count; ++i )
	{
		ec = SQLGetDiagRec( handle_type, handle, i, sql_state, &sql_error_code, error_buffer, sizeof( error_buffer ) / sizeof( SQLCHAR ), &text_length );
		FATAL_ASSERT( ec == SQL_SUCCESS );

		Errors.push_back( SODBCError( sql_error_code, (char *)sql_state, IP::String( ( char * )( error_buffer ) ) ) );
...
void CODBCObjectBase::Push_User_Error( DBErrorStateType error_state, const IP::String &error_description )
{
	...
	Errors.clear();
	Errors.push_back( SODBCError( -1, "*USER*", error_description ) );
}

void CODBCObjectBase::Log_Error_State_Base( void ) const
{
	IP::String state_string;
	...
	if ( Errors.size() > 0 )
	{
		...
		for ( uint32_t i = 0; i < Errors.size(); ++i )
		{
			...
			LOG( ELogLevel::LL_LOW, "\t\t" << i << " - EC: " << error_record.SQLErrorCode << ", SQLState: " << error_record.SQLState << ", Desc: " << error_record.ErrorDescription );
		}
	}
	else
	{
