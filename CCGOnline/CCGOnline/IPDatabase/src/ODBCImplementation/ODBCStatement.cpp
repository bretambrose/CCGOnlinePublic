...
to retry the entire batch one by one, which is slow.  The aggressive approach is to conditionally use the row number value (whose validity is conditional
...
and may fail in the face of poorly-built store procs.
*/
...
void CODBCStatement::Initialize( const IP::String &statement_text )
{
...
void CODBCStatement::Bind_Input( IDatabaseVariableSet *param_set, uint32_t param_set_size )
{
	...
	IP::Vector< IDatabaseVariable * > parameters;
...
void CODBCStatement::Bind_Output( IDatabaseVariableSet *result_set, uint32_t result_set_size, uint32_t result_set_count )
{
	...
	IP::Vector< IDatabaseVariable * > result_columns;
	...
	if ( result_columns.size() > 0 )
	{
		...
		FATAL_ASSERT( result_set_count > 0 );
		RowStatuses = IP::New_Array< SQLSMALLINT >( MEMORY_TAG, result_set_count );
...
void CODBCStatement::Execute( uint32_t batch_size )
{
	...
	error_code = SQLExecDirect( StatementHandle, (SQLCHAR *)StatementText.c_str(), SQL_NTS );
...
EFetchResultsStatusType CODBCStatement::Fetch_Results( int64_t &rows_fetched )
{
	...
	if ( CurrentResultSetWidth == -1 )
	{
		...
		{
			IP::OStringStream message_stream;
			message_stream << "\tGot a result set of width " << CurrentResultSetWidth << " while expecting a result set of width " << ExpectedResultSetWidth;
...
{
	LOG( ELogLevel::LL_LOW, "\tStatement Text: " << StatementText.c_str() );
