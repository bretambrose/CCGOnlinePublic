...
class CVirtualProcessBase : public IManagedVirtualProcess
{
	public:
		...
		CVirtualProcessBase( const SThreadKey &key );
		...
		virtual void Send_Virtual_Process_Message( const SThreadKey &dest_key, const shared_ptr< const IVirtualProcessMessage > &message );
	...
	private:
		...
		void Handle_Add_Mailbox_Message( const SThreadKey &key, const shared_ptr< const CAddMailboxMessage > &message );
		void Handle_Release_Mailbox_Request( const SThreadKey &key, const shared_ptr< const CReleaseMailboxRequest > &request );

		void Handle_Shutdown_Mailboxes( void );

		// Type definitions
		typedef stdext::hash_map< SThreadKey, shared_ptr< CWriteOnlyMailbox >, SThreadKeyContainerHelper > MailboxTableType;
		...
		shared_ptr< CWriteOnlyMailbox > LogMailbox;

		shared_ptr< CReadOnlyMailbox > MyMailbox;

		std::set< SThreadKey, SThreadKeyContainerHelper > ShutdownMailboxes;
