...
TEST( TaskSchedulerTests, Submit )
{
	...
	auto task1 = IP::Make_Shared< CMockScheduledTask >( MEMORY_TAG, 1.0 );
	...
	auto task2 = IP::Make_Shared< CMockScheduledTask >( MEMORY_TAG, 0.5 );
...
TEST( TaskSchedulerTests, Remove )
{
	...
	{
		tasks[ i ] = IP::Make_Shared< CMockScheduledTask >( MEMORY_TAG, static_cast< double >( i + 1 ) );
...
{
	auto scheduler = IP::Make_Unique< CTaskScheduler >( MEMORY_TAG );
...
TEST( TaskSchedulerTests, Granularity )
{
	...
	auto task1 = IP::Make_Shared< CMockScheduledTask >( MEMORY_TAG, 0.0 );
	scheduler.Submit_Task( task1 );
	ASSERT_TRUE( task1->Get_Execute_Time() == 0.0 );

	auto task2 = IP::Make_Shared< CMockScheduledTask >( MEMORY_TAG, 0.01 );
	scheduler.Submit_Task( task2 );
	ASSERT_TRUE( task2->Get_Execute_Time() == 1.0 );

	auto task3 = IP::Make_Shared< CMockScheduledTask >( MEMORY_TAG, 0.999 );
	scheduler.Submit_Task( task3 );
	ASSERT_TRUE( task3->Get_Execute_Time() == 1.0 );

	auto task4 = IP::Make_Shared< CMockScheduledTask >( MEMORY_TAG, 1.0 );
	scheduler.Submit_Task( task4 );
	ASSERT_TRUE( task4->Get_Execute_Time() == 1.0 );

	auto task5 = IP::Make_Shared< CMockScheduledTask >( MEMORY_TAG, 1.1 );
	scheduler.Submit_Task( task5 );
	ASSERT_TRUE( task5->Get_Execute_Time() == 2.0 );

	auto task6 = IP::Make_Shared< CMockScheduledTask >( MEMORY_TAG, 1.99 );
	scheduler.Submit_Task( task6 );
	ASSERT_TRUE( task6->Get_Execute_Time() == 2.0 );

	auto task7 = IP::Make_Shared< CMockScheduledTask >( MEMORY_TAG, 2.0 );
	scheduler.Submit_Task( task7 );
	ASSERT_TRUE( task7->Get_Execute_Time() == 2.0 );

	auto task8 = IP::Make_Shared< CMockScheduledTask >( MEMORY_TAG, 2.5 );
...
TEST( TaskSchedulerTests, Reschedule )
{
	...
	auto task = IP::Make_Shared< CMockRescheduledTask >( MEMORY_TAG, 1.0 );
