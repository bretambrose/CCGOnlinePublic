...
using ExecuteProcessDelegateType = std::function< void( EProcessID, double ) >;
...
class CProcessRecord
{
	...
	private:
		...
		IP::UniquePtr< CProcessMailbox > Mailbox;
		...
		IP::Set< EProcessID > PendingShutdownIDs;
};
...
CProcessRecord::CProcessRecord( const std::shared_ptr< IManagedProcess > &process, const ExecuteProcessDelegateType &execute_delegate ) :
	...
	Process( process ),
	Mailbox( IP::Make_Unique< CProcessMailbox >( MEMORY_TAG, process->Get_ID(), process->Get_Properties() ) ),
	ExecuteTask( nullptr ),
	ExecuteDelegate( execute_delegate ),
...
CProcessRecord::CProcessRecord( EProcessID process_id ) :
	ProcessID( process_id ),
	Process( nullptr ),
	Mailbox( IP::Make_Unique< CProcessMailbox >( MEMORY_TAG, process_id, MANAGER_PROCESS_PROPERTIES ) ),
	ExecuteTask( nullptr ),
	ExecuteDelegate(),
	State( EInternalProcessState::INITIALIZING ),
	PendingShutdownIDs()
...
void CProcessRecord::Add_Execute_Task( const std::shared_ptr< CTaskScheduler > &task_scheduler, double execution_time )
{
	...
	{
		ExecuteTask = IP::Make_Shared< CExecuteProcessScheduledTask >( MEMORY_TAG, ExecuteDelegate, Get_Process_ID(), execution_time );
	}
	else
	{
...
CConcurrencyManager::CConcurrencyManager( void ) :
	...
	TaskScheduler( IP::Make_Shared< CTaskScheduler >( MEMORY_TAG ) ),
	TimeKeeper( IP::Make_Unique< CTimeKeeper >( MEMORY_TAG ) ),
	TBBTaskSchedulerInit( IP::Make_Unique< tbb::task_scheduler_init >( MEMORY_TAG ) ),
...
void CConcurrencyManager::Enumerate_Processes( IP::Vector< std::shared_ptr< IManagedProcess > > &processes ) const
{
	processes.clear();
...
void CConcurrencyManager::Setup_For_Run( const std::shared_ptr< IManagedProcess > &starting_process )
{
	...
	ProcessRecords[ EProcessID::CONCURRENCY_MANAGER ] = IP::Make_Shared< CProcessRecord >( MEMORY_TAG, EProcessID::CONCURRENCY_MANAGER );
...
void CConcurrencyManager::Add_Process( const std::shared_ptr< IManagedProcess > &process, EProcessID id )
{
	...
	std::shared_ptr< CProcessRecord > process_record = IP::Make_Shared< CProcessRecord >( MEMORY_TAG, process, std::bind( &CConcurrencyManager::Execute_Process, this, std::placeholders::_1, std::placeholders::_2 ) );
...
void CConcurrencyManager::Handle_Ongoing_Mailbox_Requests( CProcessMailbox *mailbox )
{
	...
	for ( auto iter = PersistentGetRequests.cbegin(), end = PersistentGetRequests.cend(); iter != end; ++iter )
	{
		...
		{
			auto message = IP::Make_Process_Message< Messaging::CAddMailboxMessage >( MEMORY_TAG, Get_Mailbox( new_id ) );
...
void CConcurrencyManager::Send_Process_Message( EProcessID dest_process_id, IP::UniquePtr< Messaging::IProcessMessage > &message )
{
	IP::UniquePtr< const Messaging::IProcessMessage > const_message( message.release() );

	auto iter = PendingOutboundFrames.find( dest_process_id );
	if ( iter == PendingOutboundFrames.cend() )
	{
		auto frame = IP::Make_Unique< CProcessMessageFrame >( MEMORY_TAG, EProcessID::CONCURRENCY_MANAGER );
		frame->Add_Message( const_message );
		PendingOutboundFrames.insert( FrameTableType::value_type( dest_process_id, std::move( frame ) ) );
		return;
	}

	iter->second->Add_Message( const_message );
}


void CConcurrencyManager::Flush_Frames( void )
{
	IP::Vector< EProcessID > sent_frames;
...
void CConcurrencyManager::Service_Shutdown( void )
{
	if ( ProcessRecords.size() == 2 && State != EConcurrencyManagerState::SHUTTING_DOWN_PHASE2 )
	{
		...
		auto message = IP::Make_Process_Message< Messaging::CShutdownSelfRequest >( MEMORY_TAG, true );
...
{
	IP::Vector< IP::UniquePtr< CProcessMessageFrame > > control_frames;
	Get_My_Mailbox()->Remove_Frames( control_frames );

	// iterate all frames
	for ( uint32_t i = 0; i < control_frames.size(); ++i )
	{
		IP::UniquePtr< CProcessMessageFrame > &frame = control_frames[ i ];
...
void CConcurrencyManager::Handle_Message( EProcessID source_process_id, IP::UniquePtr< const Messaging::IProcessMessage > &message )
{
...
void CConcurrencyManager::Register_Handler( const std::type_info &message_type_info, IP::UniquePtr< Messaging::IProcessMessageHandler > &handler )
{
...
void CConcurrencyManager::Handle_Get_Mailbox_By_ID_Request( EProcessID source_process_id, IP::UniquePtr< const Messaging::CGetMailboxByIDRequest > &message )
{
	...
	{
		// fulfill the request
		auto message = IP::Make_Process_Message< Messaging::CAddMailboxMessage >( MEMORY_TAG, record->Get_Mailbox()->Get_Writable_Mailbox() );
		Send_Process_Message( source_process_id, message );
	}
}


void CConcurrencyManager::Handle_Get_Mailbox_By_Properties_Request( EProcessID source_process_id, IP::UniquePtr< const Messaging::CGetMailboxByPropertiesRequest > &message )
{
	// don't handle messages while shutting down
	if ( Is_Manager_Shutting_Down() )
	{
	...
	for ( auto iter = ProcessRecords.cbegin(), end = ProcessRecords.cend(); iter != end; ++iter )
	{
		...
		{
			auto message = IP::Make_Process_Message< Messaging::CAddMailboxMessage >( MEMORY_TAG, Get_Mailbox( iter->first ) );
			Send_Process_Message( source_process_id, message );
		}
	}
...
void CConcurrencyManager::Handle_Add_New_Process_Message( EProcessID source_process_id, IP::UniquePtr< const Messaging::CAddNewProcessMessage > &message )
{
	if ( Is_Manager_Shutting_Down() )
	{
		return;
	...
	if ( !Is_Process_Shutting_Down( source_process_id ) )
	{
		...
		{
			auto response_message = IP::Make_Process_Message< Messaging::CAddMailboxMessage >( MEMORY_TAG, Get_Mailbox( message->Get_Process()->Get_ID() ) );
			Send_Process_Message( source_process_id, response_message );
		}

		if ( message->Should_Forward_Creator_Mailbox() )
		{
			auto response_message = IP::Make_Process_Message< Messaging::CAddMailboxMessage >( MEMORY_TAG, Get_Mailbox( source_process_id ) );
...
void CConcurrencyManager::Handle_Shutdown_Process_Message( EProcessID /*source_process_id*/, IP::UniquePtr< const Messaging::CShutdownProcessMessage > &message )
{
	if ( Is_Manager_Shutting_Down() )
	{
		return;
...
void CConcurrencyManager::Handle_Reschedule_Process_Message( EProcessID source_process_id, IP::UniquePtr< const Messaging::CRescheduleProcessMessage > &message )
{
...
void CConcurrencyManager::Handle_Release_Mailbox_Response( EProcessID source_process_id, IP::UniquePtr< const Messaging::CReleaseMailboxResponse > &response )
{
	if ( Is_Manager_Shutting_Down() )
	{
		return;
	...
	if ( !record->Has_Pending_Shutdown_IDs() )
	{
		...
		auto shutdown_request = IP::Make_Process_Message< Messaging::CShutdownSelfRequest >( MEMORY_TAG, false );
		Send_Process_Message( shutdown_process_id, shutdown_request );
	}
}


void CConcurrencyManager::Handle_Shutdown_Self_Response( EProcessID source_process_id, IP::UniquePtr< const Messaging::CShutdownSelfResponse > & /*message*/ )
{
...
void CConcurrencyManager::Handle_Shutdown_Manager_Message( EProcessID /*source_process_id*/, IP::UniquePtr< const Messaging::CShutdownManagerMessage > & /*message*/ )
{
	if ( Is_Manager_Shutting_Down() )
	{
		return;
	...
	for ( auto iter = ProcessRecords.cbegin(), end = ProcessRecords.cend(); iter != end; ++iter )
	{
		...
		auto shutdown_thread_msg = IP::Make_Process_Message< Messaging::CShutdownSelfRequest >( MEMORY_TAG, true );
...
void CConcurrencyManager::Initiate_Process_Shutdown( EProcessID process_id )
{
	...
	for ( auto iter = ProcessRecords.cbegin(), end = ProcessRecords.cend(); iter != end; ++iter )
	{
		...
		auto release_mailbox_msg = IP::Make_Process_Message< Messaging::CReleaseMailboxRequest >( MEMORY_TAG, process_id );
...
void CConcurrencyManager::Log( IP::String &&message )
{
	if ( State != EConcurrencyManagerState::SHUTTING_DOWN_PHASE2 )
	{
		auto log_request = IP::Make_Process_Message< Messaging::CLogRequestMessage >( MEMORY_TAG, MANAGER_PROCESS_PROPERTIES, std::move( message ) );
