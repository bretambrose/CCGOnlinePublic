...
CProcessBase::CProcessBase( const SProcessProperties &properties ) :
	...
	MessageHandlers(),
	TaskScheduler( IP::Make_Unique< CTaskScheduler >( MEMORY_TAG ) )
{
}
...
void CProcessBase::Log( IP::String &&message )
{
	// actual logging thread should override this function and never call the baseclass
	FATAL_ASSERT( ID != EProcessID::LOGGING );

	auto log_request = IP::Make_Process_Message< Messaging::CLogRequestMessage >( MEMORY_TAG, Properties, std::move( message ) );
...
void CProcessBase::Send_Process_Message( EProcessID dest_process_id, IP::UniquePtr< Messaging::IProcessMessage > &message )
{
...
void CProcessBase::Send_Process_Message( EProcessID dest_process_id, IP::UniquePtr< Messaging::IProcessMessage > &&message )
{
	IP::UniquePtr< Messaging::IProcessMessage > const_message( message.release() );
	...
	if ( dest_process_id == EProcessID::CONCURRENCY_MANAGER )
	{
		...
		{
			ManagerFrame.reset( IP::New< CProcessMessageFrame >( MEMORY_TAG, ID ) );
		}
		
		ManagerFrame->Add_Message( std::move( const_message ) );
	}
	else if ( dest_process_id == EProcessID::LOGGING )
	{
		if ( LogFrame.get() == nullptr )
		{
			LogFrame.reset( IP::New< CProcessMessageFrame >( MEMORY_TAG, ID ) );
		}
		
		LogFrame->Add_Message( std::move( const_message ) );
	}
	else
	{
		...
		{
			auto frame = IP::Make_Unique< CProcessMessageFrame >( MEMORY_TAG, ID );
			frame->Add_Message( std::move( const_message ) );
			PendingOutboundFrames.insert( FrameTableType::value_type( dest_process_id, std::move( frame ) ) );
			return;
		}

		iter->second->Add_Message( std::move( const_message ) );
	}
}


void CProcessBase::Send_Manager_Message( IP::UniquePtr< Messaging::IProcessMessage > &message )
{
...
void CProcessBase::Handle_Shutdown_Mailboxes( void )
{
	for ( auto iter = ShutdownMailboxes.cbegin(), end = ShutdownMailboxes.cend(); iter != end; ++iter )
	{
		...
		auto release_msg = IP::Make_Process_Message< Messaging::CReleaseMailboxResponse >( MEMORY_TAG, process_id );
...
void CProcessBase::Handle_Message( EProcessID process_id, IP::UniquePtr< const Messaging::IProcessMessage > &message )
{
...
void CProcessBase::Register_Handler( const std::type_info &message_type_info, IP::UniquePtr< Messaging::IProcessMessageHandler > &handler )
{
...
void CProcessBase::Handle_Add_Mailbox_Message( EProcessID source_process_id, IP::UniquePtr< const Messaging::CAddMailboxMessage > &message )
{
	IP_UNREFERENCED_PARAM( source_process_id );
...
void CProcessBase::Handle_Release_Mailbox_Request( EProcessID source_process_id, IP::UniquePtr< const Messaging::CReleaseMailboxRequest > &request )
{
...
void CProcessBase::Handle_Shutdown_Self_Request( EProcessID source_process_id, IP::UniquePtr< const Messaging::CShutdownSelfRequest > &message )
{
	FATAL_ASSERT( source_process_id == EProcessID::CONCURRENCY_MANAGER );
	...
	auto shutdown_self_msg = IP::Make_Process_Message< Messaging::CShutdownSelfResponse >( MEMORY_TAG );
...
void CProcessBase::Build_Process_ID_List_By_Properties( const SProcessProperties &properties, IP::Vector< EProcessID > &process_ids ) const
{
	process_ids.clear();
