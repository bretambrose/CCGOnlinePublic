...
TEST_F( ODBCMiscTests, AllocateAndCleanup )
{
	...
	IDatabaseStatement *statement = connection->Allocate_Statement( "{call dynamic.get_all_accounts()}" );
...
class CDummyTask : public CDatabaseTaskBase
{
	public:
		...
		virtual const char *Get_Database_Object_Name( void ) const { return ""; }
...
class CSignatureGoodSelectTask : public CDummyTask
{
	public:
		...
		virtual void Build_Column_Name_List( IP::Vector< const char * > &column_names ) const 
		{
			column_names.push_back( "test1" );
			column_names.push_back( "test2" );
			column_names.push_back( "test3" );
			column_names.push_back( "test4" );
		}
