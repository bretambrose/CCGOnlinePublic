/**********************************************************************************************************************
	...
	ProcessMessageHandlerTests.cpp
		defines unit tests for process message handler related functionality
...
class CMockMessageHandlerTracker
{
	public:
		...
		void Handle_Message( EProcessID::Enum process_id, const shared_ptr< const IProcessMessage > &message )
		{
	...
	private:
		...
		void Handle_Log_Request( EProcessID::Enum /*process_id*/, const shared_ptr< const CLogRequestMessage > &message )
		{
			LastLogMessage = message->Get_Message();
		}

		void Handle_Shutdown_Process_Request( EProcessID::Enum /*process_id*/, const shared_ptr< const CShutdownProcessMessage > &message )
		{
			LastShutdownProcessID = message->Get_Process_ID();
		}

		stdext::hash_map< Loki::TypeInfo, shared_ptr< IProcessMessageHandler >, STypeInfoContainerHelper > MessageHandlers;
...
TEST( ProcessMessageHandlerTests, Register_And_Handle )
{
	...
	tracker.Handle_Message( EProcessID::CONCURRENCY_MANAGER, shared_ptr< const IProcessMessage >( new CLogRequestMessage( MANAGER_PROCESS_PROPERTIES, LOG_MESSAGE_1 ) ) );
	ASSERT_TRUE( tracker.Get_Last_Log_Message() == LOG_MESSAGE_1 );

	tracker.Handle_Message( EProcessID::CONCURRENCY_MANAGER, shared_ptr< const IProcessMessage >( new CLogRequestMessage( LOGGING_PROCESS_PROPERTIES, LOG_MESSAGE_2 ) ) );
	ASSERT_TRUE( tracker.Get_Last_Log_Message() == LOG_MESSAGE_2 );

	tracker.Handle_Message( EProcessID::CONCURRENCY_MANAGER, shared_ptr< const IProcessMessage >( new CShutdownProcessMessage( EProcessID::LOGGING ) ) );
	ASSERT_TRUE( tracker.Get_Last_Shutdown_Process_ID() == EProcessID::LOGGING );

	tracker.Handle_Message( EProcessID::CONCURRENCY_MANAGER, shared_ptr< const IProcessMessage >( new CShutdownProcessMessage( EProcessID::CONCURRENCY_MANAGER ) ) );
