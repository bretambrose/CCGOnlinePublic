...
class CProcessBaseTester
{
	public:
		...
		void Log( const IP::String &log_string );
		...
		const IP::UniquePtr< IP::Execution::CProcessMessageFrame > &Get_Frame( IP::Execution::EProcessID id ) const;
		...
		const IP::UniquePtr< IP::Execution::CProcessMessageFrame > &Get_Log_Frame( void ) const;
		const IP::UniquePtr< IP::Execution::CProcessMessageFrame > &Get_Manager_Frame( void ) const;
...
class CThreadProcessBaseTester : public CProcessBaseTester
{
	...
	private:
		...
		IP::UniquePtr< std::thread > ProcessThread;
